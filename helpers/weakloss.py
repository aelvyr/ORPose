...
import helpers.bmc_config as cfg
...
class BMCLoss:
    def __init__(
        ...
        with open(os.path.join(self.lp, "CONVEX_HULLS.pkl"), 'rb') as f:
            self.joint_angle_limit = pickle.load(f)
    ...
    def compute_loss(self, joints):
        ...
        if self.lambda_a:
            ...
            # TIP bones
            if len(DIP_X_axis.shape) == 2:
                DIP_X_axis = DIP_X_axis.unsqueeze(0)
                DIP_Y_axis = DIP_Y_axis.unsqueeze(0)
